...
Start_state = pygame_gui.elements.UITextEntryLine(relative_rect=report_rect,
                                                  manager=manager)

### Start state label
pygame_gui.elements.ui_label.UILabel(parent_element=Start_state,
                                     manager=manager,
                                     text="Start State:", # (pos-width,pos-height),(width,height)
                                     relative_rect=pygame.Rect((855, 210), (140, 30)))

### set start state with button
        ...
        if event.type == pygame.USEREVENT:
            if event.user_type == pygame_gui.UI_BUTTON_PRESSED:
                if event.ui_element == shuffle_button:
                    ...
                    print ("fstate",fstate)
                    if not puzzle.setBlocks(Start_state.get_text()):
                        alert_label.set_text("Starting state invalid!")
                    else:
                        alert_label.set_text("Starting state valid!")
                        puzzle.start_state = Start_state.get_text()
                        print ("fstate",fstate)
            ...
            elif event.user_type == pygame_gui.UI_TEXT_ENTRY_CHANGED and event.ui_element == Start_state:
                print("")
